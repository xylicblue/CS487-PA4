...
**App Service** hosts the stateless Node.js frontend because it handles HTTP scaling, TLS, and custom domains out of the box with no container orchestration overhead and also allows direct CI/CD integration with multiple provider (in our case it was github). A simple web server does not need the complexity of Kubernetes or the cold-start penalty of serverless. Continuing on the first point, it is the right choice for our webapp because it is a PaaS and we dont need to handle the webapp configuration manually.

**Durable Functions** manages the multi-step order workflow because it persists orchestration state to storage automatically, provides built-in retry and checkpointing across the validate→report chain, and resumes from the last completed step after any crash or timeout. It is important to note here that all of this happens without any requirement from the developer to write state saving code (so minimal manual code implementation required)
...
**ACI** handles the one-shot PDF report job because it runs exactly once per order and then exits. ACI bills only for the seconds the container runs, making it cost-efficient for a background task with no idle traffic. Spinning up a dedicated pod in AKS for a job that exits in under a minute would waste a persistent node slot because we only have to generated a PDF once its validated by the orchestrator (through the aks) and so why should we keep it on for the entire time rather than just spinning it up when we needed the pdf generated. Saves us on cost while also fulfiling the requirements of the task at hand.

### Question 8.3: ACI vs AKS

**Idle behavior:** AKS keeps its node pool running at all times, even with zero orders, the VM incurs a continuous hourly charge. ACI has no idle cost as the container group is created on demand and deleted after completion, so there is zero cost between orders.

**Cost behavior:** AKS cost is dominated by node VM pricing (fixed per hour regardless of load). ACI cost is purely consumption-based which means that the user gets billed per vCPU-second and GB-second of actual container runtime, which is negligible for a sub-60-second report job.

**Operational model:** AKS provides a persistent, stateful deployment with health checks, rolling updates, and horizontal scaling making it appropriate for the role of always-on validator. ACI spins up when needed - the container is created fresh for each job, removing any state management burden. For the validator, AKS's persistent model ensures sub-second response times, for the report job, ACI's on-demand model eliminates the need to manage a standing deployment.

### Question 8.4: Durable Functions vs Plain HTTP

**Problem 1 — State persistence across crashes:** A plain HTTP call chain has no memory. If the Function App process crashes or times out between the validate call and the ACI report job, the entire order is lost and must be resubmitted manually. Durable Functions checkpoints after every `yield`, so a restart replays only from the last completed activity, the validate result is not re-fetched and the ACI is not double-spawned which saves us on the cost incurred by ACI double spawning.
...
**Challenge 1 — Subscription policy blocking shared-key storage access:** The non-production subscription enforced `allowSharedKeyAccess: false` on all storage accounts via Azure Policy, which silently broke both the Durable Functions runtime (which defaults to a connection string for `AzureWebJobsStorage`) and the report job blob upload. The error only showed as a `403 AuthorizationFailure` after role assignments were confirmed correct. The fix required switching entirely to Managed Identity authentication: setting `AzureWebJobsStorage__accountName`, `AzureWebJobsStorage__credential=managedidentity`, and `AzureWebJobsStorage__clientId` for the Functions runtime, and rewriting `generate.py` to use `ManagedIdentityCredential` instead of a connection string. Since this was not mentioned in the handout at the time I started my assignment, I had to contact the TA for this and then he gave the permissions on each MI through the issue got resolved.

**Challenge 2 — Function App returning 401/503 after restart due to rotated keys and storage misconfiguration:** After fixing the storage account to use identity-based auth and restarting the Function App, the HTTP starter began returning 401 Unauthorized even though the function key retrieved via CLI looked correct. The root cause was that `AzureWebJobsSecretStorageType` was set to `Files`, meaning the running container stored its function keys in the local filesystem, while the Azure CLI read keys from blob storage, two completely separate keystores that diverged after every restart. The fix was to set `AzureWebJobsSecretStorageType=blob` so both the running host and the CLI read from the same blob-backed keystore. Even after this, the default function key returned by the CLI was not accepted, and I had to fall back to the master host key to confirm the endpoint was alive before the correct key was propagated. Additionally, the reports blob container initially had public network access disabled, which meant the ACI report job could upload the PDF successfully but the returned `report_url` would return `PublicAccessNotPermitted` when opened in a browser. Enabling public network access on the storage account resolved this and allowed the PDF URL to be verified.
